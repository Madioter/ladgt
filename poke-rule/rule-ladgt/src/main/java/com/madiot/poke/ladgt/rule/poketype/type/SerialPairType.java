...
/**
 * @ClassName: SERIAL_PAIR
 * @Description: TODO
 * @author Yi.Wang2
 * @date 2017/8/18
 */
public class SerialPairType extends BasePokeTypeRule {

    @Override
    public <T extends IPokeCard> boolean check(IOneHand<T> oneHand) {
        List<T> array = oneHand.getCards();
        if (CollectionUtils.isEmpty(array) || array.size() <= 6 || array.size() % 2 != 0) {
            return false;
        } else {
    ...
    public String getType() {
        return PokeType.SERIAL_PAIR.toString();
    }
}
